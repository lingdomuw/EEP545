...
class ReSampler:
  ...
  def resample_naiive(self):
    ...
    # YOUR CODE HERE
    particles_range=range(n_particles)
    temp=self.particles.copy()
    indexes = np.random.choice(particles_range, size=n_particles, replace=True, p=self.weights)
    self.particles[:] = [temp[i] for i in indexes]
  ...
  def resample_low_variance(self):
    ...
    n_particles = len(self.particles)
    temp=self.particles.copy()
    r = np.random.uniform(0,1/float(n_particles))
    c = self.weights[0]
    i = 0
