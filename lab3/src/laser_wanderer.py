...
class LaserWanderer:
  ...
  '''
  def __init__(self, rollouts, deltas,delta_incr, speed, compute_time, laser_offset, T,car_width):
  ...
  def compute_cost(self, delta, rollout_pose, laser_msg):
    ...
    car_pose = np.array([0.0,0.0,0.0], dtype=np.float) # initial car pose
    rollout_angle = np.arctan((rollout_pose[1] - car_pose[1])/(rollout_pose[0] - car_pose[0]))
    #rospy.loginfo(rollout_angle)
    #rospy.loginfo(len(laser_msg.ranges))

    rollout_vector=np.array([rollout_pose[0],rollout_pose[1]])
    theta=rollout_angle 
    #T = np.array([[np.cos(theta), -np.sin(theta)],[np.sin(theta), np.cos(theta)]]) #Rotation Matrix
    if delta>=0:
      car_edge_pose=np.array([0.0-self.width/2,0.0,0.0])
    else:
      car_edge_pose=np.array([self.width/2,0.0,0.0])
    
    if self.width==0.0:

      rollout_distance = np.linalg.norm(np.array(car_pose[:-1]) - np.array(rollout_pose[:-1]))
    else:
      rollout_distance = np.linalg.norm(np.array(car_edge_pose[:-1]) - np.array(rollout_pose[:-1]))

    #rospy.loginfo(rollout_distance)
    # check if the rollout is on left or on right of the car


    if theta < laser_msg.angle_min or theta > laser_msg.angle_max:
      cost += MAX_PENALTY
    else: 
      angle_index = int(round((theta - laser_msg.angle_min) / laser_msg.angle_increment))
      #right_angle_index = int(round((0 - laser_msg.angle_min) / laser_msg.angle_increment))
      #left_angle_index = int(round((np.pi - laser_msg.angle_min) / laser_msg.angle_increment))

      #rospy.loginfo(laser_msg.angle_min)
      #rospy.loginfo(laser_msg.angle_max)
      
      if angle_index  < len(laser_msg.ranges):
          laser_dist =laser_msg.ranges[angle_index]
          #left_dist=abs(laser_msg.ranges[-1]) # car left laser measurement
          #right_dist=abs(laser_msg.ranges[int(len(laser_msg.ranges) / 2)] )# car right laser measurement
          #rospy.loginfo(laser_dist)
          #rospy.loginfo(left_dist)
          #rospy.loginfo(right_dist)

          # If laser measurement is NAN or 0, we use the previous laser measurement
          if (np.isnan(laser_dist)) or laser_dist < 0:
            cost+=MAX_PENALTY
          #rospy.loginfo(laser_dist)

          # Add MAX_PENALTY to the cost if the distance from the robot to the rollout_pose 
          # is greater than the laser ray measurement - np.abs(self.laser_offset)
          if rollout_distance > (laser_dist-np.abs(self.laser_offset)):# or \
            #(self.width/2 > (left_dist-np.abs(self.laser_offset))) or \
            #(self.width/2 > (right_dist-np.abs(self.laser_offset))):
...
def generate_rollout(init_pose, controls, car_length):
  ...
  rollouts = np.asarray(rollout_list)
...
def main():
  ...
  car_width = rospy.get_param("~car_width",0.0) # the width of the car, Default val:0.0 in simulation
  ...
  lw = LaserWanderer(rollouts, deltas,delta_incr, speed, compute_time, laser_offset, T,car_width)
