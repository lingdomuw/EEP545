...
BAD_WAYPOINTS_CSV='/waypoints/real_car/bad_waypoints.csv'
		...
		with open(csv_path) as csv_file: 
			...
			for row in csv_reader:
				if count == 0:
					...
					bad_x = int(row[0])
					bad_y = int(self.mapHeight-int(row[1]))
					boundries=numpy.array([bad_x-half_length, bad_x+half_length,bad_y-half_length,bad_y+half_length])
					#print(boundries)
					self.mapImageBW[boundries[2]:boundries[3],boundries[0]:boundries[1]] = 255
