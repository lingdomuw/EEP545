...
class SensorModel:
  ...
  def lidar_cb(self, msg):
    ...
    obs_ranges = np.array(msg.ranges[::self.LASER_RAY_STEP], dtype=np.float32 )
    #assert len(self.downsampled_angles) == len(obs_ranges),"Dimension Mismatching Error!"

    obs_ranges[np.isnan(obs_ranges)] = self.MAX_RANGE_METERS
    obs_ranges[obs_ranges==0.0] = self.MAX_RANGE_METERS

    obs=(obs_ranges,self.downsampled_angles)
    assert obs[0].shape[0] == obs[1].shape[0],"Dimension Mismatching Error!"
  ...
  # calculation probabilities with measured and expeced ranges
  def prob_calculate(self,ztk, ztk_star, zmax):
    ztk = np.float32(ztk)
    ...
    if 0.0 <= ztk < zmax:
      ...
      normalizer_hit = 1.0 
      p_hit = p_hit / normalizer_hit 
    else:
      p_hit = 0.0 

    # 6.7
    if 0.0 <= ztk < ztk_star:
      lambda_short = 0.001 # make lambda_short a very small number
    ...
    # 6.13
    Z= np.array([Z_HIT,Z_SHORT,Z_MAX,Z_RAND])
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    for d in xrange(table_width):
      for r in xrange(table_width):
        p = self.prob_calculate(r,d, max_range_px+1)     
        sensor_model_table[r,d] = p

    #normalize propabilites
