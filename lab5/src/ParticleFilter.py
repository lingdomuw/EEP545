...
class ParticleFilter():
  ...
  def expected_pose(self):
    ...
    expected_x = np.sum([self.weights[i] * self.particles[i][0] for i in range(len(self.particles))])
    expected_y = np.sum([self.weights[i] * self.particles[i][1] for i in range(len(self.particles))])
    expected_theta = np.arctan2(np.sum([np.sin(self.particles[i][2]) for i in range(len(self.particles))]), np.sum([np.cos(self.particles[i][2]) for i in range(len(self.particles))]))
  ...
  def clicked_pose_cb(self, msg):
    self.state_lock.acquire()
    rospy.loginfo("clicked pose callback function")
    ...
    pose_theta = Utils.quaternion_to_angle(msg.pose.pose.orientation) 

    map_pose = np.array((pose_x, pose_y, pose_theta), ndmin = 2 )
    ...
    for i in range(len(self.particles)):
      ...
      y_sample = 0
      while not in_bounds:
        x_sample = int (np.random.normal(map_pose[:,0], .005) )# good std deviation?
        y_sample = int (np.random.normal(map_pose[:,1], .005) ) # good std deviation?
        in_bounds = self.permissible_region[y_sample][x_sample]
        # rospy.loginfo(x_sample) 
        # rospy.loginfo(y_sample)

      self.particles[i] = [x_sample,y_sample,map_pose[:,2]]
